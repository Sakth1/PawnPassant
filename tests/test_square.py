...
class TestSquareConstruction(unittest.TestCase):
    ...
    def test_highlight_metadata_default(self):
        sq = Square(file=2, rank=2, coordinate="c3", color=chess.BLACK)
...
class TestSquareHighlight(unittest.TestCase):
    def test_set_highlight_updates_metadata(self):
        ...
        self.assertEqual(sq.highlighted_metadata["parent_piece_square"], "e4")
...
class TestSquareSetCoordinatesVisible(unittest.TestCase):
    ...
    def test_flipped_coordinates(self):
        sq = Square(file=7, rank=7, coordinate="h8", color=chess.BLACK)
...
class TestInvisibleSquare(unittest.TestCase):
    def test_creates_with_color(self):
        inv = InvisibleSquare(
            coordinate=0, color=chess.WHITE, drag_drop_group="test-group"
        )
    ...
    def test_default_has_no_piece(self):
        inv = InvisibleSquare(
            coordinate=1, color=chess.BLACK, drag_drop_group="test-group"
        )
    ...
    def test_background_is_invisible(self):
        inv = InvisibleSquare(
            coordinate=2, color=chess.WHITE, drag_drop_group="test-group"
        )
    ...
    def test_drag_data_encoding(self):
        inv = InvisibleSquare(
            coordinate=3, color=chess.WHITE, drag_drop_group="test-group"
        )
        data = inv._drag_data()
