...
class JsonFileSettingsBackend:
    ...
    async def save(self, payload: dict[str, Any]) -> None:
        ...
        logger.info("Settings saved path=%s", self.file_path)
...
class SettingsController:
    ...
    def update(self, **changes: Any) -> AppSettings:
        ...
        logger.info("Settings updated keys=%s", ",".join(sorted(changes.keys())))
        bus.emit(SettingsChangedEvent(self.settings))
        self._schedule_save()
        return self.settings
    ...
    async def _build_backend(self) -> SettingsBackend | None:
        ...
        if platform_name == "android" and storage is not None:
            logger.info("Using shared-preferences settings backend platform=android")
        ...
        if platform_name == "windows":
            ...
            if support_dir is not None:
                ...
                logger.info("Using JSON settings backend platform=windows")
                return backend

        if storage is not None:
            logger.info(
                "Using shared-preferences settings backend platform=%s",
                platform_name,
            )
            return SharedPreferencesSettingsBackend(storage, self.STORAGE_KEY)
        ...
        support_dir = await self._resolve_support_directory()
        if support_dir is not None:
            logger.info("Using JSON settings backend support_dir=%s", support_dir)
            return JsonFileSettingsBackend(support_dir / self.FILE_NAME)

        logger.info("No settings backend available")
        return None
    ...
    async def _migrate_shared_preferences_to_file(
        ...
        if legacy_payload is not None:
            ...
            logger.info("Migrated legacy settings to file path=%s", backend.file_path)
